...
void solve() {
  int n, m;
  cin >> m >> n;
  vector<vector<char>> avion(n, vector<char>(m));
  ...
  int minDisparos = INT_MAX;
  for (auto [n, disparos] : freq) {
    if (disparos < minDisparos) {
      candidatos = {n};
      minDisparos = disparos;
    } else if (disparos == minDisparos) {
  ...
  for (int candidato : candidatos) {
    if (size[candidato] < minZona) {
      minZona = size[candidato];
      final = candidato;
    }
  }
  cout << final << endl;
