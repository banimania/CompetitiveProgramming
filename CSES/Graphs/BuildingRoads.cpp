...
void solve() {
  ...
  vector<int> connectedComponents; // store the first element of a connected component (representative node)
  for (int i = 0; i < n; i++) {
    if (!visited[i]) {
      visited[i] = true;
  ...
  for (int i = 1; i < connectedComponents.size(); i++) {
    cout << connectedComponents[i - 1] + 1 << " " << connectedComponents[i] + 1 << endl;
  }
}
