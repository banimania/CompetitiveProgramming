...
void solve() {
  ...
  priority_queue<State, vector<State>, function<bool(State, State)>> pq([](const State &a, const State &b) {
    if (a.cost == b.cost) return a.usedDiscount > b.usedDiscount;
    return a.cost > b.cost;
  });
  vector<vector<ll>> minCost(n, vector<ll>(2, LLONG_MAX)); // minCost[ciudad][descuentoUsado]
  pq.push({0, 0, false});
  minCost[0][0] = 0;
  minCost[0][1] = 0;
  while (!pq.empty()) {
    ...
    if (current.cost > minCost[current.city][current.usedDiscount]) continue;
    ...
    for (auto [neighbour, cost] : adj[current.city]) {
      if (!current.usedDiscount) {
        ll newDist = current.cost + cost / 2;
        if (newDist < minCost[neighbour][1]) {
          minCost[neighbour][1] = newDist;
          pq.push({neighbour, newDist, true});
        }
      }

      ll newDist = current.cost + cost;
      if (newDist < minCost[neighbour][current.usedDiscount]) {
        minCost[neighbour][current.usedDiscount] = newDist;
        pq.push({neighbour, newDist, current.usedDiscount});
      }
    }
  }
}
