...
void dfs(int current, int parent, int length, vector<vector<int>> &adj, vector<int> &prev, vector<bool> &visited, bool &solved) {
  if (solved) return;
  
  if (visited[current] && length >= 3) {
    solved = true;

    int sol = current;
    vector<int> solution = { current + 1 };
    current = prev[current];

    while (current != -1 && current != sol) {
      solution.push_back(current + 1);
      current = prev[current];
    }
  ...
  for (int neighbour : adj[current]) {
    ...
    dfs(neighbour, current, length + 1, adj, prev, visited, solved);
  }
...
void solve() {
  ...
  for (int i = 0; i < n && !solved; i++) {
    if (!visited[i]) {
      visited[i] = true;
      dfs(i, -1, 0, adj, prev, visited, solved);
    }
  }
