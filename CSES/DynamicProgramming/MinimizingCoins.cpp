...
// dp[i] = -1 if impossible
// dp[i] = LLONG_MAX if not computed yet
ll minCoins(ll x, vll &dp, vll &coins) {
  if (dp[x] != LLONG_MAX) return dp[x];

  for (int i = 0; i < coins.size(); i++) {
    if (x - coins[i] >= 0 && minCoins(x - coins[i], dp, coins) != -1) {
      dp[x] = min(dp[x], minCoins(x - coins[i], dp, coins) + 1);
    } 
  }

  return dp[x] = (dp[x] == LLONG_MAX ? -1 : dp[x]);
}
