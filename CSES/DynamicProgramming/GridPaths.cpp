...
ll countWays(ll i, ll j, vector<vector<char>> &a, vector<vector<ll>> &dp) {
  if (i < 0 || j < 0) return 0;
  if (dp[i][j] != -1) return dp[i][j];
  if (a[i][j] == '*') return dp[i][j] = 0;

  return dp[i][j] = (countWays(i - 1, j, a, dp) + countWays(i, j - 1, a, dp)) % MOD;
}
...
void solve() {
  ...
  cout << countWays(n - 1, n - 1, a, dp) << endl;
}
