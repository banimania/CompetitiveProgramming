...
void solve() {
  ...
  set<ll> precios;
  for (int i = 0; i < n; i++) {
    set<ll> nuevosPrecios(precios.begin(), precios.end());
    ll precioI = a[i];
    nuevosPrecios.insert(precioI);
    for (ll precio : precios) {
      nuevosPrecios.insert(precio + precioI);
    }
    precios = nuevosPrecios;
  }
 
  cout << precios.size() << endl;
  for (ll precio : precios) {
    cout << precio << " ";
  }
  cout << endl;
