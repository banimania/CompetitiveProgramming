...
void solve() {
  ...
  while (cin >> I) {
    ...
    priority_queue<Puente, vector<Puente>, function<bool(Puente, Puente)>> pq([](const Puente &a, const Puente &b) {
    ...
    while (!pq.empty()) {
      ...
      if (find_set(dsu, p.u) != find_set(dsu, p.v)) {
        ans += p.coste;
        union_sets(dsu, p.u, p.v);
      }
    }
