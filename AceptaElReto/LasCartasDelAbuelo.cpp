...
struct EqualOp {
  ...
  //   if (b.ignore) b.c = a.c;
  //   char c = a.c;
  //   bool isEquals = (a.equals && b.equals && a.c == b.c);
  //   return {c, isEquals};
  // }
  ...
  T operator()(T a, T b) const {
    ...
    return {a.c, a.equals && b.equals && a.c == b.c};
  }
...
void solve() {
  ...
  while (getline(cin, s)) {
    ...
    for (int i = 0; i < s.length(); i++) {
      a.push_back({s[i], true, false});
    }
    SegTree<Cell, EqualOp> segTree(a);
    while (n--) {
      int a, b;
      cin >> a >> b;

      cout << (segTree.query(min(a, b), max(a, b)).equals ? "SI" : "NO") << endl;
    }
    cin.ignore();
  }
}
