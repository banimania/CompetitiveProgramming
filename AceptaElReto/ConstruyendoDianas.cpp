...
void solve() {
  int valor, sectores;
  while (cin >> valor >> sectores) {
    vector<int> puntos(sectores), dp(valor + 1, INT_MAX), prev(valor + 1, -1);
    for (int i = 0; i < puntos.size(); i++) {
      cin >> puntos[i];
    }
    dp[0] = 0;
    sort(puntos.begin(), puntos.end(), [](const int &a, const int &b) {
      return a > b;
    });

    for (int i = 1; i < valor + 1; i++) {
      for (int j = 0; j < puntos.size(); j++) {
        if (i - puntos[j] >= 0 && dp[i - puntos[j]] != INT_MAX) {
          if (dp[i - puntos[j]] + 1 < dp[i]) {
            prev[i] = puntos[j];
            dp[i] = dp[i - puntos[j]] + 1;
          }
        }
      }
    }

    int minDardos = dp[valor];

    if (minDardos == INT_MAX) {
      cout << "Imposible" << endl;
    } else {
      cout << minDardos << ": ";
      vector<int> solucion;
      int current = valor;
      while (current != 0) {
        solucion.push_back(prev[current]);
        current -= prev[current];
      }

      sort(solucion.begin(), solucion.end(), [](const int &a, const int &b) {
        return a > b;
      });

      for (int i = 0; i < solucion.size(); i++) {
        cout << solucion[i];
        if (i + 1 < solucion.size()) cout << " ";
      }
      cout << endl;
    }
  }
