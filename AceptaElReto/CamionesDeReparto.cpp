...
void solve() {
  ...
  while (cin >> v >> e) {
    ...
    for (int i = 0; i < e; i++) {
      ...
      cin >> u >> v >> ancho;
      u--, v--;
      adj[u].push_back({v, ancho});
      adj[v].push_back({u, ancho});
    }
    int k;
    cin >> k;
    for (int i = 0; i < k; i++) {
      ...
      while (!q.empty()) {
        ...
        for (pair<int, int> calle : adj[actual]) {
          if (calle.second < ancho || visited[calle.first]) continue;

          visited[calle.first] = true;
          q.push(calle.first);
        }
      }

      cout << (posible ? "SI" : "NO") << endl;
    }
  }
}
