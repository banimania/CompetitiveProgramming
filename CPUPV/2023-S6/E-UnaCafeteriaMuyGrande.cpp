...
void solve() {
  ...
  vector<queue<string>> colas(n, queue<string>());
  REP (i, 0, m) {
    char c;
    cin >> c;
    ...
    if (c == 'a') {
      ...
      if (colas[x].empty()) {
        cout << "- No hay nadie en la fila " << x + 1 << endl;
      } else {
        cout << "- Atendiendo a " << colas[x].front() << endl;
        colas[x].pop();
      }
    } else if (c == 'p') {
      cout << "- Personas en la fila " << x + 1 << ": " << colas[x].size() << endl;
    }
  }
  ...
  cout << "Personas por atender: " << tot << endl;
}
