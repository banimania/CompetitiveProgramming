...
struct MSAOp {
  ...
  T operator()(T a, T b) const {
    if (a.end == b.start) {
      ll lengthMax = b.end - a.start;
      if (lengthMax > a.max && lengthMax > b.max) return {a.start, b.end, lengthMax};
    }

    if (a.max > b.max) return {a.start, a.end, a.max};
    else return {b.start, b.end, b.max};
  }
...
void solve() {
  ...
  map<string, ll> registros;
  stack<ll> pila;

  while (currentInstruction < n) {
    istringstream iss(instr[currentInstruction]);
    iss >> instruction;
    if (instruction == "halt") break;
      ...
      ll dir;
      iss >> reg >> dir;
      if (registros[reg] > 0) registros[reg]--;
      else if (registros[reg] == 0) {
        currentInstruction = dir;
        continue;
      }
      continue;
      ...
      currentInstruction = pila.top();
      pila.pop();
      continue;
    } else if (instruction == "call") {
      ll dir;
      iss >> dir;
      pila.push(currentInstruction + 1);
      currentInstruction = dir;
      continue;
