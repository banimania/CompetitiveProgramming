...
void solve() {
  ...
  vector<vector<ll>> lago(n, vector<ll>(m));
  REP (i, 0, n) {
    REP (j, 0, m) {
      cin >> lago[i][j];
    }
  ...
  REP (i, 0, n) {
    REP (j, 0, m) {
      if (lago[i][j] && !visitados.count({i, j})) {
        q.push({i, j});
        ...
        while (!q.empty()) {
          ...
          sumaLago += lago[actual.first][actual.second];
          for (pair<ll, ll> dir : dirs) {
            ...
            if (newPos.first >= 0 && newPos.first < n && newPos.second >= 0 && newPos.second < m) {
              if (lago[newPos.first][newPos.second] && !visitados.count(newPos)) {
