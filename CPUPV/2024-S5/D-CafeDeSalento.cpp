...
void solve() {
  ll n;
  cin >> n;
  vector<pair<ll, string>> cosas(n);
  REP (i, 0, n) {
    cin >> cosas[i].first >> cosas[i].second;
  }

  sort(cosas.begin(), cosas.end(), [](const pair<ll, string> &a, const pair<ll, string> &b) {
    if (a.first == b.first) {
  ...
  REP (i, 0, n) {
    cout << cosas[i].first << " " << cosas[i].second << endl;
  }
}
