...
// set<pair<ll, ll>> dirs = {{0, -1}, {0, 1}, {-1, 0}, {1, 0}};
// void solve() {
//   ll n, m;
//   cin >> n >> m;

//   vector<vector<char>> a(n, vector<char>(m));
//   set<pair<ll, ll>> lobos;
//   REP (i, 0, n) {
//     REP (j, 0, m) {
//       cin >> a[i][j];
//       if (a[i][j] == '.') a[i][j] = 'P';
//       else if (a[i][j] == 'L') lobos.insert({i, j});
//     }
//   }

//   set<pair<ll, ll>> visitados;
//   for (pair<ll, ll> lobo : lobos) {
//     queue<pair<ll, ll>> q;
//     q.push(lobo);
//     visitados.insert(lobo);
//     while (!q.empty()) {
//       pair<ll, ll> actual = q.front();
//       q.pop();

//       if (a[actual.first][actual.second] == 'O') {
//         cout << "No" << endl;
//         return;
//       }

//       for (pair<ll, ll> dir : dirs) {
//         pair<ll, ll> newPos = {actual.first + dir.first, actual.second + dir.second};
//         if (newPos.first >= 0 && newPos.first < n && newPos.second >= 0 && newPos.second < m) {
//           if (a[newPos.first][newPos.second] == 'P') continue;
//           q.push(newPos);
//           visitados.insert(newPos);
//         }
//       }
//     }
//   }
...
void solve() {
  ...
  vector<vector<char>> a(n, vector<char>(m));
  vector<pair<ll, ll>> lobos;
  REP (i, 0, n) {
    REP (j, 0, m) {
      cin >> a[i][j];
      if (a[i][j] == '.') a[i][j] = 'P';
      else if (a[i][j] == 'L') lobos.push_back({i, j});
    }
  }

  for (pair<ll, ll> lobo : lobos) {
    for (pair<ll, ll> dir : dirs) {
      pair<ll, ll> newPos = {lobo.first + dir.first, lobo.second + dir.second};
      if (newPos.first >= 0 && newPos.first < n && newPos.second >= 0 && newPos.second < m) {
        if (a[newPos.first][newPos.second] == 'O') {
          cout << "No" << endl;
          return;
        }
      }
