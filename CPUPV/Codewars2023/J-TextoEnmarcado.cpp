...
void solve() {
  ...
  vector<vector<string>> textos(n);

  REP (i, 0, n) {
    ll m;
    cin >> m;

    textos[i] = vector<string>(m);
    cin.ignore();
  ...
  vector<ll> borderSize(n);
  REP (i, 0, n) {
    ll border = 0;
    REP (j, 0, textos[i].size()) {
      border = max(border, (ll)textos[i][j].length() + 4);
    }
    borderSize[i] = border;
  }

  REP (i, 0, n) {
    ll border = borderSize[i];

    if (i == 0 || borderSize[i] > borderSize[i - 1]) {
      REP (j, 0, border) {
        cout << "#";
      }
      cout << endl;
    }
    REP (j, 0, textos[i].size() + 2) {
      cout << "#";
      REP (k, 0, border - 2) {
        if (j >= 1 && j < textos[i].size() + 1 && k >= 1 && k < border - 3) {
          if (k - 1 < textos[i][j - 1].length()) cout << textos[i][j - 1][k - 1];
          else cout << " ";
        }
        else cout << " ";
      }
    ...
    if (i == n - 1 || borderSize[i] > borderSize[i + 1]) {
      REP (j, 0, border) {
        cout << "#";
      }
      cout << endl;
