...
ll countWays(ll x, ll y, ll n, ll m, vector<vector<ll>>& a, vector<vector<ll>>& dp) {
  if (dp[x][y] != -1) return dp[x][y];
  if (x == 0 && y == 0) return dp[x][y] = 1;

  ll waysUp = 0, waysLeft = 0;

  REP (i, 0, x) {
    if (a[i][y] == x - i) {
      waysUp += countWays(i, y, n, m, a, dp);
    }
  }
  
  REP (i, 0, y) {
    if (a[x][i] == y - i) {
      waysLeft += countWays(x, i, n, m, a, dp);
    }
  }

  return dp[x][y] = (waysUp + waysLeft) % MOD;
}
...
void solve() {
  ...
  cout << countWays(n - 1, m - 1, n, m, a, dp) << endl;
}
