...
string buscar(string bus, vector<string> &files) {
  string ans;

  REP (i, 0, files.size()) {
    int pos = files[i].find(bus);
    if (pos != string::npos) {
      string lastDir = files[i].substr(0, pos);
      if (lastDir == "/") break;

      lastDir = lastDir.substr(0, lastDir.size() - 1);
      ans += buscar(lastDir, files);
      break;
    }
  }
