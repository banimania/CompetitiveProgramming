...
ll maxWay(ll x, ll y, ll n, ll m, vector<vector<ll>>& a, vector<vector<ll>>& dp) {
  if (x < 0 || y < 0) return INT_MIN;
  if (x == 0 && y == 0) return a[0][0];
  if (dp[x][y] != -1) return dp[x][y];

  return dp[x][y] = a[x][y] + max(maxWay(x - 1, y, n, m, a, dp), maxWay(x, y - 1, n, m, a, dp));
}
...
void solve() {
  ...
  cout << maxWay(n - 1, m - 1, n, m, a, dp) << endl;
}
