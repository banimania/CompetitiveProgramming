...
void solve() {
  ...
  vector<pair<ll, ll>> parFreq;
  for (pair<ll, ll> fr : freq) {
    if (fr.first != 0) parFreq.push_back(fr);
  }

  sort(parFreq.begin(), parFreq.end(), [](const pair<ll, ll> &a, const pair<ll, ll> &b) {
    return a.second > b.second;
  });
  cout << parFreq[2].first << " " << parFreq[1].first - 1 << endl;
}
