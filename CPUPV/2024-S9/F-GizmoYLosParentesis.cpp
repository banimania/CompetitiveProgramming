...
struct Nodo {
  ll numOpen, numClose, numMatched;
};
...
struct ParOp {
  ...
  T operator()(T a, T b) const {
    ll numMatched = min(a.numOpen, b.numClose);
    
    return { a.numOpen + b.numOpen - numMatched, a.numClose + b.numClose - numMatched, a.numMatched + b.numMatched + numMatched };
  }
...
void solve() {
  ...
  REP (i, 0, s.length()) {
    if (s[i] == '(') nodos.push_back({1, 0, 0});
    else if (s[i] == ')') nodos.push_back({0, 1, 0});
  }

  SegTree<Nodo, ParOp> segTree(nodos);
  ...
  REP (i, 0, q) {
    ...
    cin >> l >> r;
    l--, r--;
    cout << segTree.query(l, r).numMatched * 2 << endl;
  }
}
