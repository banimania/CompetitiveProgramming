...
void solve() {
  ...
  vector<vector<pair<ll, ll>>> ady(n, vector<pair<ll, ll>>());
  ...
  vector<ll> riesgoMin(n, LLONG_MAX);
  vector<vector<ll>> mejorCamino(n);
  riesgoMin[s] = 0;
  pq.push({s, 0, {s}});
  ...
  while (!pq.empty()) {
    ...
    for (auto [vecino, riesgo] : ady[actual.pos]) {
      ll newRiesgo = actual.riesgo + riesgo;
      if (riesgoMin[vecino] > newRiesgo) {
        riesgoMin[vecino] = newRiesgo;
        State newState = actual;
        newState.riesgo = newRiesgo;
        newState.pos = vecino;
        newState.camino.push_back(vecino);
        pq.push(newState);
        mejorCamino[vecino] = newState.camino;
      }
    }
  }

  REP (i, 0, n) {
    if (i == s || riesgoMin[i] == LLONG_MAX) continue;
    cout << i << ": " << endl;
    cout << "riesgo " << riesgoMin[i] << endl;

    vll camino = mejorCamino[i];

    cout << mejorCamino[i][camino.size() - 2];
   
    REPV (j, camino.size() - 3, 0) {
