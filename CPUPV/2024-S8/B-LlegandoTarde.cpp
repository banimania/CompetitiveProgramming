...
void solve() {
  ...
  while (!pq.empty()) {
    ...
    if (actual.pos == y) {
      cout << actual.steps << endl;
      cout << actual.camino << endl;
      return;
    }

    for (ll vecino : ady[actual.pos]) {
      ll newDist = actual.steps + 1;
      if (dist[vecino] >= newDist) {
        dist[vecino] = newDist;
        State newState = actual;
        newState.camino += to_string(vecino + 1) + " ";
        newState.steps = newDist;
        newState.pos = vecino;
        pq.push(newState);
      }
    }
