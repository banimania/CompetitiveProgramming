...
void bfs(ll n, ll t, ll v, ll s, set<ll>& sol, vector<vector<pair<ll, ll>>>& ady) {
  // v = s / t -> s = v * t
  ll maxDist = v * t;
  ...
  while (!q.empty()) {
    ...
    for (auto [vecino, vDist] : ady[pos]) {
      ll newDist = dist + vDist;

      if (maxDist >= newDist && !visitados[vecino]) {
        visitados[vecino] = true;
        q.push({vecino, newDist});
      }
    }
  }
}
...
void solve() {
  ...
  vector<vector<pair<ll, ll>>> ady(n);
  REP (i, 0, n - 1) {
    ll u, v, d;
    cin >> u >> v >> d;
    u--, v--;
  ...
  set<ll> plazasA, plazasB;

  bfs(n, t, va, a, plazasA, ady);
  bfs(n, t, vb, b, plazasB, ady);

  set<ll> plazasSol;
  for (ll plazaA : plazasA) {
    if (plazasB.count(plazaA)) {
      plazasSol.insert(plazaA);
    }
  }

  cout << plazasSol.size() << endl;
  for (ll sol : plazasSol) cout << sol + 1 << " ";
  cout << endl;
} 
