...
void dibujar(ll n, vector<vector<char>> &dib, ll aux) {
  if (n == 0) return;
  ...
  REP (i, aux, aux + alto) {
    REP (j, aux + i, 3 * aux + ancho - i) {
      dib[i][j] = n + '0';
    }
  }

  dibujar(n - 1, dib, aux + 1);
}
...
void solve() {
  ...
  ll ancho = 3 + 4 * (n - 1);
  ll alto = 2 * n;
  vector<vector<char>> dib(alto, vector<char>(ancho, ' '));

  dibujar(n, dib, 0);

  reverse(dib.begin(), dib.end());

  REP (i, 0, alto) {
    REP (j, 0, ancho) {
      if (j > 0 && dib[i][j] == ' ' && dib[i][j - 1] != ' ') break;
      cout << dib[i][j];
    }
    cout << endl;
  }
