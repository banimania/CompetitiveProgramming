...
void solve() {
  ...
  string horasInput, minutosInput;
  bool colon = false;
  REP (i, 0, s.size()) {
    if (s[i] == ':') {
      colon = true;
    ...
    if (!colon) horasInput += s[i];
    else minutosInput += s[i];
  }

  int horas = stoi(horasInput);
  int minutos = stoi(minutosInput);
  ...
  horas %= 24;
  string horasString, minutosString;
  if (horas < 10) {
    horasString += '0';
  }
  if (minutos < 10) {
  ...
  horasString += to_string(horas);
  minutosString += to_string(minutos);

  // cout << "PELIGRO DE INUNDACIÓN: Evita entrar a sótanos y bajos, evita desplazamientos y busca un lugar alto. La inundación llegará a las " << horasString << ":" << minutosString << endl;
  cout << "PELIGRO DE INUNDACION: Evita entrar a sotanos y bajos, evita desplazamientos y busca un lugar alto. La inundacion llegara a las " << horasString << ":" << minutosString << endl;
}
