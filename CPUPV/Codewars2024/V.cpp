...
#define PRINT(v) for (int i = 0; i < v.size(); i++) { auto x = v[i]; cout << v[i]; if (i != v.size() - 1) {cout << " ";} } cout << endl;
...
ll buscarCoordenada(ll coord, vll &dim) {
  ll alto = dim[coord];
  ll bajo = 0;
 
  vll consulta = {0, 0, 0, dim[0], dim[1], dim[2]};
  while (alto - bajo > 1) {
    ll medio = (alto - bajo) / 2 + bajo;
 
    consulta[coord] = bajo;
    consulta[coord + 3] = medio;
    cout << "? ";
    PRINT(consulta);
...
void solve() {
  ...
  x = buscarCoordenada(0, dim);
  y = buscarCoordenada(1, dim);
  z = buscarCoordenada(2, dim);
