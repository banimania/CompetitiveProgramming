...
ll maxLen(ll i, ll k, vll &a, vll &prefixSum, vll &dp) {
  if (i < 0) return 0;
  ...
  // unirse
  if (i - 1 >= 0) {
    ll newLen = maxLen(i - 1, k, a, prefixSum, dp) + 1;
    ll newSum = prefixSum[i] - (i - newLen >= 0 ? prefixSum[i - newLen] : 0);    
    if (newSum <= k) {
...
void solve() {
  ...
  ll ans = 0;
  REP (i, 0, n) {
    ans = max(ans, maxLen(i, k, a, prefixSum, dp));
  }
