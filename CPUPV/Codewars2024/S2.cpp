...
void solve() {
  ...
  REP (i, 0, n) {
    ...
    if (isalpha(x)) {
      if (!freq.count(x)) orden.push_back(x);
      freq[x]++;
    }
  }

  char maxLetra = orden[0], minLetra = orden[0];
  ll maxLetraC = freq[orden[0]], minLetraC = freq[orden[0]];
  REP (i, 1, orden.size()) {
    char letra = orden[i];
    ll veces = freq[letra];
  ...
  cout << maxLetra << ": " << maxLetraC << endl;
  cout << minLetra << ": " << minLetraC << endl;
}
