...
void solve() {
  ...
  while (!q.empty()) {
    ...
    if (actual.first == n - 1 && actual.second == m - 1) {
      buenos++;
      continue;
    }
    if (mapa[actual.first][actual.second] == 'X') {
      malos++;
      continue;
    }

    if (actual.first + 1 < n) q.push({actual.first + 1, actual.second});
    if (actual.second + 1 < m) q.push({actual.first, actual.second + 1});
  }
