...
ll obamium(string &s, ll inicio, ll fin) {
  if (inicio > fin) return 0;
  else if (inicio == fin) return s[inicio] == 'O';

  bool onlyObamium = true;
  REP (i, inicio, fin + 1) {
    if (s[i] != 'O') {
      onlyObamium = false;
      break;
  ...
  if (onlyObamium) return fin - inicio + 1;

  ll mid = (fin - inicio) / 2 + inicio;
  if ((fin - inicio) % 2 == 0) mid--;
  return max(obamium(s, inicio, mid), obamium(s, mid + 1, fin));
}
...
void solve() {
  ...
  cout << obamium(s, 0, s.length() - 1) << endl;
}
