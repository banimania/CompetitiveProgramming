...
void solve() {
  ...
  vector<int> maxValidRow(n, -1), maxValidColumn(m, -1);
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
      if (a[i][j] == 1) {
        bool possible = false;
        ...
        for (int k = i; k >= 0; k--) {
          if (maxValidColumn[j] >= k) {
            maxValidColumn[j] = i;
            possible = true;
            break;
          }
          if (!a[k][j]) break;
          if (k == 0) possible = true, maxValidColumn[j] = i;
        }
        if (!possible) {
          ...
          for (int k = j; k >= 0; k--) {
            ...
            if (!a[i][k]) break;
            if (k == 0) possible = true, maxValidRow[i] = j;
          }
        }
  ...
  //         if ((current.first > 0 && a[current.first - 1][current.second] == 0) && (current.second < m - 1 && a[current.first][current.second + 1] == 0)) {
  //           impossible = true;
  //         }

  //         for (pair<ll, ll> dir : dirs) {
  //           ll newI = current.first + dir.first;
  //           ll newJ = current.second + dir.second;

  //           if (newI >= 0 && newI < n && newJ >= 0 && newJ < m && a[newI][newJ] && !visited[newI][newJ]) {
