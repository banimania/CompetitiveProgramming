...
void solve() {
  ...
  vll sorted(a.begin(), a.end());
  SORTINC(sorted);

  vll prefixSum(n), prefixSumSorted(n);
  prefixSum[0] = a[0];
  prefixSumSorted[0] = sorted[0];
  REP (i, 1, n) {
    prefixSum[i] = prefixSum[i - 1] + a[i];
    prefixSumSorted[i] = prefixSumSorted[i - 1] + sorted[i];
  }
  ...
  REP (i, 0, m) {
    ...
    ll ans;
    if (type == 1) {
      ans = (prefixSum[r] - (l - 1 >= 0 ? prefixSum[l - 1] : 0));
    } else {
      ans = (prefixSumSorted[r] - (l - 1 >= 0 ? prefixSumSorted[l - 1] : 0));
    }
    cout << ans << endl;
  }
}
