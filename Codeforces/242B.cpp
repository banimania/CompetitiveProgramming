...
void solve() {
  ...
  vector<tuple<ll, ll, ll>> segments;
  ...
  sort(segments.begin(), segments.end(), [](const tuple<ll, ll, ll> &a, const tuple<ll, ll, ll> &b) {
    if (get<0>(a) == get<0>(b)) return get<1>(a) > get<1>(b);
    return get<0>(a) < get<0>(b);
  });

  if (get<1>(segments[0]) < maxR) {
    cout << -1 << endl;
    return;
  }

  cout << get<2>(segments[0]) << endl;
