...
void solve() {
  ll n;
  cin >> n;
  vector<vector<ll>> ady(n, vector<ll>(n));
  ...
  sort(grados.begin(), grados.end(), [&](const pair<ll, ll>& a, const pair<ll, ll>& b) {
    return (a.first < b.first) ? ady[a.first][b.first] : !ady[b.first][a.first];
  });

  for (pair<ll, ll> p : grados) {
    cout << p.first + 1 << " ";
  }
  cout << endl;
}
