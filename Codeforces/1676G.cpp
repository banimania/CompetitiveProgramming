...
pair<ll, ll> dfs(ll current, ll &cnt, vector<vector<ll>> &adj, string &colors) {
  ll blacks = 0, whites = 0;
  if (colors[current] == 'W') whites++;
  else blacks++;

  for (ll neighbour : adj[current]) {
    pair<ll, ll> c = dfs(neighbour, cnt, adj, colors);
    blacks += c.first;
    whites += c.second;
  }

  if (blacks == whites && blacks) {
    cnt++;
  }
