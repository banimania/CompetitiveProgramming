...
void solve() {
  ...
  vector<vector<ll>> ady(n, vector<ll>());
  map<ll, set<ll>> colores;
  REP (i, 0, m) {
    ll u, v, c;
    cin >> u >> v >> c;
    u--, v--, c--;
  ...
  queue<pair<ll, ll>> q; // nodo y dist
  vector<bool> visitados(n, false);
  set<ll> coloresVisitados;
  visitados[b] = true;
  q.push({b, 0});
  while (!q.empty()) {
    ...
    for (ll color : ady[actual.first]) {
      if (coloresVisitados.count(color)) continue;
      for (ll vecino : colores[color]) {
        if (!visitados[vecino]) {
          visitados[vecino] = true;
          q.push({vecino, actual.second + 1});
        }
      }
      coloresVisitados.insert(color);
    }
