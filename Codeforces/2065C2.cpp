...
void solve() {
  ...
  REP (i, 0, n) {
    ll oldValue = LLONG_MAX;
    if (i == 0 || a[i] >= a[i - 1]) oldValue = a[i];

    ll newValue = LLONG_MAX;
    ll prev = (i - 1 >= 0 ? a[i - 1] : LLONG_MIN);
    ll index = lower_bound(b.begin(), b.end(), prev + a[i]) - b.begin();
    if (index != b.size()) {
      newValue = b[index] - a[i];
    }

    a[i] = min(newValue, oldValue);
    if (a[i] == LLONG_MAX) {
      cout << "NO" << endl;
      return;
    }
