...
void solve() {
  ...
  int aOnesEvens = 0, aOnesOdds = 0, bZerosEvens = 0, bZerosOdds = 0;
  for (int i = 0; i < n; i++) {
    if (i % 2) {
      if (a[i] == '1') aOnesOdds++;
      if (b[i] == '0') bZerosOdds++;
      ...
      if (a[i] == '1') aOnesEvens++;
      if (b[i] == '0') bZerosEvens++;
    }
  }

  if (bZerosEvens >= aOnesOdds && bZerosOdds >= aOnesEvens) {
    cout << "YES" << endl;
    return;
  }
