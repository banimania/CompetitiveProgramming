...
template <typename T>
vector<T> prefixSum(const vector<T>& arr) {
  int n = arr.size();
  vector<T> pref(n + 1, 0);
  for (int i = 1; i <= n; i++) {
...
/*
int count(vector<int> &a, vector<int> &b, vector<int> &pSumA, vector<int> &pSumB) {
  int ans = 0;

  REP (i, 0, a.size()) {
    REP (j, i, a.size()) {
      int sumA = pSumA[j] - (i - 1 >= 0 ? pSumA[i - 1] : 0);
      int sumB = pSumB[j] - (i - 1 >= 0 ? pSumB[i - 1] : 0);
      if (sumA == sumB) ans++;
    }
  }
...
void solve() {
  ...
  cout << a[0] << endl;
  // vector<int> a = {4, 7, 5, 1, 2, 6, 3};
  // vector<int> b = {7, 5, 1, 2, 6, 3, 4};

  // vector<int> pSumA = prefixSum(a);
  // vector<int> pSumB = prefixSum(b);
  // cout << count(a, b, pSumA, pSumB) << endl;
}
