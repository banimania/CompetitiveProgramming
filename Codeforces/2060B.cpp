...
void solve() {
  ...
  vector<pair<ll, vector<ll>>> cartas;
  REP(i, 0, n) {
    vector<ll> cart(m);
    REP(j, 0, m) cin >> cart[j];
    SORTINC(cart);
    cartas.push_back({i, cart});
  }

  sort(cartas.begin(), cartas.end(), [](const pair<ll, vector<ll>>& a, const pair<ll, vector<ll>>& b) {
  ...
  REP (round, 0, m) {
    REP (i, 0, n) {
      ll cowIndex = cartas[i].first;
      while (di[i] < m && cartas[i].second[di[i]] < currentCard + 1) di[i]++;
      ...
      currentCard = cartas[i].second[di[i]];
      di[i]++;
    }
